...
Public Sub Initialize
	...
	If GetDevicePhysicalSize < 6 Then
		cScaleTxt = Min(cScaleX, cScaleY)
	Else
		cScaleTxt = Max(cScaleX, cScaleY)
	End If
...
'Gets the current Txt scale with the Rate set with SetRate 
'or with the default Rate value of 0.3
Public Sub GetScaleTxt As Float
	Return cScaleTxt
End Sub
...
Public Sub ScaleView(v As View)
	...
	If v Is Label Then 'this will catch ALL views with text (EditText, Button, ...)
		Dim lbl As Label = v
		lbl.TextSize = lbl.TextSize * cScaleTxt
	End If
	...
	Else If GetType(v) = "anywheresoftware.b4a.objects.SpinnerWrapper$B4ASpinner" Then
		...
		Dim spn As Spinner
		spn = v
		spn.TextSize = spn.TextSize * cScaleTxt
	End If
End Sub
...
Public Sub SetReferenceLayout(Width As Int, Height As Int, theScale As Double)
